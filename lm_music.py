...
def train(dim_word=100,  # word vector dimensionality
    ...
    worddicts = collections.OrderedDict()
    for  i in range(8000):
        worddicts[str(i)] = i
    ...
    valid = TextIterator(valid_dataset,
                         ...
                         maxlen=maxlen)
    '''
    train_d = Data(mb_size = 32, seq_length = 4000, audio_path = '/data/lisatmp4/anirudhg/SpeechSyn/1k_rate.wav')
    valid_d = Data(mb_size = 32, seq_length = 4000, audio_path ='/data/lisatmp4/anirudhg/SpeechSyn/1k_validation_sample.wav')
    ...
    for eidx in xrange(max_epochs):
        ...
        for kk in range(0, 800000):
            x = train_d.getBatch()
            x = train_d.digitize(x)
            n_samples += len(x)
            uidx += 1
            ...
            d = Data(mb_size = 32, seq_length = 4000, audio_path = '/data/lisatmp4/anirudhg/SpeechSyn/1k_rate.wav')
            print sampleFreq
            if numpy.mod(uidx, sampleFreq) == 0:
                output = numpy.zeros(4000)
                for jj in xrange(5):
                    sample, score = gen_sample(tparams, f_next,
                                               model_options, trng=trng,
                                               maxlen=4000, argmax=False)
                    print 'Sample ', jj, ': ',
                    ss = sample
                    counter = 0
                    for vv in ss:
                        ...
                        if vv in worddicts_r:
                            output[counter]= int(worddicts_r[vv])
                    ...
                    gen_wave = d.dedigitize(output)
                    d.saveExample(gen_wave, "sample_gen_" +  str(jj))
                    d.saveExampleWav(gen_wave, "sample_gen_wave_" + str(jj))
