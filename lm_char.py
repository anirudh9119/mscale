...
def init_params(options):
    ...
    #temp = numpy.zeros((options['n_words'], options['n_words']), int).astype('float32')
    #numpy.fill_diagonal(temp,1)
    #temp = temp.astye('float32')
    #params['Wemb'] = temp
...
def pred_probs(f_log_probs, prepare_data, options, iterator, maxlen, verbose=False):
    probs = []
    n_done = 0
    leng = []
    for x in iterator:
        n_done += len(x)
        x, x_mask = prepare_data(x, maxlen,  n_words=options['n_words'])
       # x_mask = numpy.ones_like(x).astype('float32')
        w = float(x_mask.sum(0).mean())
        leng.append(w)
    ...
    leng = numpy.asarray(leng)
    print '%d average length computed' % (leng.sum(0)/len(leng))
...
def train(dim_word=100,  # word vector dimensionality
    ...
    '''
    data = numpy.load('/data/lisa/data/PennTreebankCorpus/char_level_penntree.npz', 'rb')
    numtrain, numvalid, numtest = (data['train'].shape[0]/batch_size)*batch_size, data['valid'].shape[0], data['test'].shape[0]


    num_examples = numtrain/(batch_size * maxlen)
    data_train = data['train'][0:num_examples*batch_size*maxlen]

    num_valid_examples = numvalid/(batch_size * maxlen)
    data_valid = data['valid'][0:num_valid_examples*batch_size*maxlen]

    train = numpy.reshape(data_train,[num_examples, maxlen, batch_size])
    valid = numpy.reshape(data_valid,[num_valid_examples, maxlen, batch_size])
    '''
    ...
    for eidx in xrange(max_epochs):
        ...
        print 'Seen %d samples, %d epoch' % (n_samples,eidx)
